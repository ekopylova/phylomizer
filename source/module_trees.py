...
from module_utils import lookForFile
...
def phylogenetic_trees(parameters):
  ...
  for approach in tree_approaches:
    ...
    for model in selected_models:
      ...
      if not log_lk:
        print(("ERROR: Impossible to the Log likelihood values "
          + "for '%s' model using this program '%s'") % (model, prog), file=sys.stderr)
    ...
    if not lookForFile(rank_file) or replace or update:
      ...
      print("\n".join([("%s\t%s") % (r[0], r[1]) for r in records]), file=out_file)
      out_file.close()
...
def perform_tree(label, binary, parameters, in_file, out_file, stats_file, \
  ...
  print(("###\n###\t%s - Phylogenetic Trees\t") % (label.upper()), end=' ', file=logFile)
  print(("%s\n###\t[%s]\tCommand-line\t%s\n###") % (date, name, cmd), file=logFile)
  logFile.flush()
  ...
  if label in ["phyml", "codonphyml"]:
    ...
    try:
      ...
      print(("ERROR: Impossible to rename RAxML output files"), file=sys.stderr)
      sys.exit(exit_codes[label])
