...
from module_utils import parseComments, sort_blast_hits
...
def homology(parameters):
  ...
  if not lookForFile(outFile) or parameters["replace"]:
    ...
    print(("%s\t%s") % (parameters["prefix"], seqs_md5), file=open(outFile, "w"))
  ...
  if not lookForFile(outFile) or parameters["replace"]:
    ...
    for seqId in sorted(selected_sequences):
      print((">%s\n%s") % (seqId, selected_sequences[seqId][1]), file=output_file)
    output_file.close()
